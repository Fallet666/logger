        ...
        for (size_t i = 0; i < this->format_string.size(); ++i) {
            if (this->format_string[i] == '%' && i + 1 < this->format_string.size()) {
                switch (this->format_string[++i]) {
                    case 'L': result += toString(level, use_colors);
                        break;
                    case 'T': result += fmt::format("{:%H:%M:%S}",
                                                    fmt::localtime(
                                                        std::chrono::system_clock::to_time_t(
                                                            std::chrono::system_clock::now())));
                        break;
                    case 'N': result += this->name;
                        break;
                    case 'M': result += message;
                        break;
                    case 't': result += std::to_string(std::hash<std::thread::id>{}(std::this_thread::get_id()));
                        break;
                    case 'S': result += file;
                        break;
                    case '#': result += std::to_string(line_number);
                        break;
                    default: result += '%';
